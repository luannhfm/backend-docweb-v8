...
module.exports = __toCommonJS(analysis_service_exports);
...
// src/services/analysis.service.ts
async function updateAnalysisStatus(jsonAnalysis, analysisId) {
  const analysisRepository = appDataSource.getRepository(Analysis);
  const result = await analysisRepository.update(analysisId, {
    status: "conclu\xEDdo",
    analysis: jsonAnalysis
  });
  if (result.affected === 0) {
    throw new Error("Nenhum registro foi atualizado. Verifique se o ID est\xE1 correto.");
  }
}
